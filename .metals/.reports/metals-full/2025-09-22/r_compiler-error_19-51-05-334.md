file:///C:/coc/moneytalk/modules/users-api/src/main/scala/usersapi/HttpServer.scala
...
action parameters:
uri: file:///C:/coc/moneytalk/modules/users-api/src/main/scala/usersapi/HttpServer.scala
text:
```scala
...
object HttpServer: // Объявляем объект сервера
  ...
  private val routes: HttpRoutes[cats.effect.IO] = HttpRoutes.of[cats.effect.IO] { // Определяем маршруты на Cats IO
    ...
    case GET -> Root / "openapi.yaml" => // Эндпоинт выдачи OpenAPI
      val endpoints = List(TapirEndpoints.machinesEndpoint) // Собираем эндпоинты
